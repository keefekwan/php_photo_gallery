...
class Photograph extends DatabaseObject {
	...
	public function destroy() {
		...
		if ($this->delete()) {
			...
			$target->path = SITE_ROOT.DS."public".DS.$this->image_path();
	...
	public function size_as_text() {
		if ($this->size < 1024) {
			...
			$size_mb = round($this->size/1048576, 1); // round to 1 decimal place
			return "{$size_mb} MB";
		}
	}
	...
	public static function find_all() {
		global $database;
		$database->query("SELECT * FROM photographs WHERE id = 27");
	...
	// 	$result_set = $database->query($sql);
	// 	$object_array = array();
	// 	while ($row = $database->resultset($result_set)) {
	// 		$object_array[] = self::instantiate($row);
	// 	}
	// 	return $object_array;
	// }
